...
3. ✅ **Migrador** `Cardiotox row → Bundle FHIR` (`src/cardiotox-mapping/migration/`): la tabla existente se migra fila a fila.
   ...
   - `migrate.ts` — CLI: `tsx migrate.ts <cardiotox.csv> [--echo f] [--ecg f] [--qt f] [--frcv f]`. Dry-run por defecto (escribe los Bundles a JSON) · `--inspect` **reporta la cobertura de encabezados sin escribir nada** (correrlo PRIMERO contra el export real) · `--limit N` acota a los primeros N pacientes (**smoke test** de la primera subida) · `--execute` sube a Medplum vía `executeBatch` · `--tab` para TSV. CSV de-identificados de ejemplo en `data/example/`.
     - **Servidor destino:** `https://api.medplum.com.ar` (Favaloro | Medplum Argentina), leído de `.env`. El default estaba apuntando a otro host — corregido, porque un `--execute` sin la variable seteada habría escrito los 327 pacientes en el servidor equivocado.
     - **Runbook de la primera subida:** `--inspect` → `--limit 5 --execute` → verificar en Medplum → corrida completa. Al ser idempotente (PUT por identifier), repetir la corrida actualiza en vez de duplicar.
