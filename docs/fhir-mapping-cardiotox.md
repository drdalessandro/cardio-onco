...
3. ✅ **Migrador** `Cardiotox row → Bundle FHIR` (`src/cardiotox-mapping/migration/`): la tabla existente se migra fila a fila.
   ...
   - `migrate.ts` — CLI: `tsx migrate.ts <archivo.csv>` (dry-run, escribe los Bundles a JSON) · `--execute` (sube a Medplum vía `executeBatch`, credenciales `MEDPLUM_CLIENT_ID/SECRET`) · `--tab` para TSV. CSV de-identificado de ejemplo en `data/example/cardiotox-sample.csv`.
   - Cobertura: 46 tests (parseo + mapeo contra fila testigo de-identificada: mujer 56, linfoma B, R-CHOP → 4 familias de quimio, SAC=Intermedio). Series temporales (Ecocardiogramas_control, Estudios_Complementarios) y capa CKM (FRCV) se unen por `DNI` en un incremento posterior.
